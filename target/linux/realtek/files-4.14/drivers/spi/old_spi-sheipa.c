				   ...
				   uint8_t wr_cmd)
{
	...
	if (!enable_addr_4byte_mode) {
		...
		spi_flash_setdr(dev, 0, wr_cmd_addr, DATA_WORD);
	} else {
		wr_cmd = CMD_FORMAT(wr_cmd);
		wr_addr = ADDR_4BYTE_FORMAT(wr_addr);
		spi_flash_setdr(dev, 0, wr_cmd, DATA_BYTE);
		spi_flash_setdr(dev, 1, wr_addr, DATA_WORD);
	}
}
...
static void flash_tx_cmd(struct sheipa_spi *dev, uint8_t cmd)
{
	...
	spi_flash_setdr(dev, 0, cmd, DATA_BYTE);
	/* Enable SPI_FLASH */
...
static void flash_set_status(struct sheipa_spi *dev, uint32_t addr, uint8_t cmd)
{
	...
	spi_flash_setdr(dev, 0, cmd, DATA_BYTE);
	/* Set data FIFO */
	spi_flash_setdr(dev, 0, data, DATA_BYTE);
...
static uint8_t flash_get_status(struct sheipa_spi *dev)
{
	...
	return spi_flash_getdr(dev, 0, DATA_BYTE);
}
			     ...
			     enum spi_flash_byte_num byte_num)
{
	if (byte_num == DATA_BYTE)
		...
		spi_flash_setdr(dev, 0, data, DATA_HALF);
	else if (byte_num == DATA_WORD)
		spi_flash_setdr(dev, 0, data, DATA_WORD);
}
...
static inline void do_spi_recv(struct sheipa_spi *dws, u8 *buf, uint32_t len)
{
	...
	if (!mode) {
		if (len <= FIFO_SIZE) {
			...
			cnt = len / 4;
			for (i = 0; i < cnt; i++) {
				data = spi_flash_getdr(dws, 0, DATA_WORD);
				memcpy(buf, &data, 4);
				buf += 4;
			}
			cnt = len % 4;
